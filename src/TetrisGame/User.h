...
public:
    ...
    static bool addUser(int fd, std::shared_ptr<User> user);

    static bool removeUser(int fd);
...
private:
    ...
    static std::map<int, std::shared_ptr<User>> users;
    static std::mutex users_mutex; // 添加互斥锁
};
