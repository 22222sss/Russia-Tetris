...
map<int, std::shared_ptr<User>> User::users;//静态成员变量定义
std::mutex User::users_mutex;
...
bool User::addUser(int fd, std::shared_ptr<User> user) 
{
    std::lock_guard<std::mutex> lock(users_mutex);
    return users.emplace(fd, user).second;
}

bool User::removeUser(int fd) 
{
    std::lock_guard<std::mutex> lock(users_mutex);
    return users.erase(fd) > 0;
}

std::shared_ptr<User> User::getUser(int fd) 
{
    std::lock_guard<std::mutex> lock(users_mutex);
...
std::map<int, std::shared_ptr<User>> User::getAllUsers()
{
    std::lock_guard<std::mutex> lock(users_mutex);
    return users; // 返回副本
}
